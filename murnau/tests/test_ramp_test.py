...
class TestMain:
    """Test the main function of ramp_test"""
    
    @patch('time.sleep')
    @patch('pythonosc.udp_client.SimpleUDPClient')
    @patch('builtins.print')
    def test_main_function_complete_flow(self, mock_print, mock_client_class, mock_sleep):
        ...
        gate_on_calls = [call for call in mock_client.send_message.call_args_list 
                        if f"/{synth_name}/gate" in call[0] and call[0][1] == 1.0]
        gate_off_calls = [call for call in mock_client.send_message.call_args_list 
        ...
        mock_print.assert_any_call("Testing ramp from 220Hz to 880Hz over 2.0s")
        mock_print.assert_any_call("Testing ramp from 880Hz to 220Hz over 2.0s")
        mock_print.assert_any_call("Testing ramp from 440Hz to 880Hz over 0.5s")
        mock_print.assert_any_call("Testing ramp from 880Hz to 440Hz over 0.5s")
...
class TestTestScenarios:
    ...
    def test_all_test_scenarios_executed(self, mock_client_class, mock_sleep):
        ...
        start_freq_calls = [call for call in mock_client.send_message.call_args_list 
                           if f"/{synth_name}/start_freq" in call[0]]
        end_freq_calls = [call for call in mock_client.send_message.call_args_list 
                         if f"/{synth_name}/end_freq" in call[0]]
        ramp_time_calls = [call for call in mock_client.send_message.call_args_list 
    ...
    def test_scenario_data_types(self):
        ...
        # Since the test scenarios are defined in main(), we verify through execution
        with patch('time.sleep'), \
             patch('pythonosc.udp_client.SimpleUDPClient') as mock_client_class:
...
class TestIntegration:
    ...
    def test_complete_test_cycle(self, mock_client_class, mock_sleep):
        ...
        # Should have: 4 init + (4 tests * 4 params) + (4 tests * 2 gate msgs) = 4 + 16 + 8 = 28
        expected_message_count = 4 + 16 + 8
        ...
        gate_calls = [call for call in mock_client.send_message.call_args_list 
    ...
    def test_real_timing_simulation(self, mock_client_class):
        ...
        # Temporarily patch the test scenarios to use very short durations
        with patch('ramp_test.main') as mock_main:
            def fast_main():
                client = mock_client_class("127.0.0.1", 5510)
                synth_name = "legato_synth_stereo"
                ...
                # One quick test
                client.send_message(f"/{synth_name}/start_freq", 440)
                client.send_message(f"/{synth_name}/end_freq", 880)
                client.send_message(f"/{synth_name}/ramp_time", 0.001)
                client.send_message(f"/{synth_name}/gate", 1.0)
                time.sleep(0.001)  # Very short
                client.send_message(f"/{synth_name}/gate", 0.0)
            
            mock_main.side_effect = fast_main
