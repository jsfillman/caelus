...
class TestMain:
    ...
    @patch('melody.init_synth')
    @patch('melody.play_note')
    @patch('melody.midi_to_freq')
    @patch('time.sleep')
    ...
    def test_main_function_complete_flow(self, mock_print, mock_client_class, mock_sleep, 
                                       mock_midi_to_freq, mock_play_note, mock_init_synth):
        """Test the complete main function flow"""
        mock_client = Mock()
        mock_client_class.return_value = mock_client
        mock_midi_to_freq.side_effect = [261.63, 329.63, 392.00, 523.25, 392.00, 329.63, 261.63]
        ...
        expected_midi_calls = [call(60), call(64), call(67), call(72), call(67), call(64), call(60)]
    ...
    @patch('melody.play_note')
    @patch('pythonosc.udp_client.SimpleUDPClient')
    def test_main_with_init_synth_exception(self, mock_client_class, mock_play_note, mock_init_synth):
        """Test main function when init_synth raises exception"""
        mock_client = Mock()
        mock_client_class.return_value = mock_client
    ...
    @patch('melody.midi_to_freq')
    @patch('pythonosc.udp_client.SimpleUDPClient')
    def test_main_with_play_note_exception(self, mock_client_class, mock_midi_to_freq, 
                                         mock_play_note, mock_init_synth):
        """Test main function when play_note raises exception"""
        mock_client = Mock()
        mock_client_class.return_value = mock_client
...
class TestMelodyData:
    ...
    def test_melody_structure(self):
        ...
        # We need to access the melody from the main function context
        # Since it's defined within main(), we test it indirectly through the calls
        with patch('melody.play_note') as mock_play_note, \
             patch('melody.init_synth'), \
             patch('melody.midi_to_freq') as mock_midi_to_freq, \
             patch('time.sleep'), \
             patch('pythonosc.udp_client.SimpleUDPClient'):
            ...
            durations = [call[0][2] for call in play_note_calls]  # Third argument is duration
...
class TestIntegration:
    ...
    def test_full_melody_integration(self, mock_client_class, mock_sleep):
        ...
        # Should have 14 init messages + 14 melody messages (7 notes * 2 messages each)
        expected_message_count = 14 + (7 * 2)  # init + (notes * (freq + gate on/off))
