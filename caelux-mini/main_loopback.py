#!/usr/bin/env python3
"""
Caelux Mini Synthesizer - Hardcoded Loopback Audio Version
This version forces the use of Loopback Audio 2 and eliminates all audio device selection.
"""
...
class MainWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Caelux Mini - Loopback Audio")
    ...
    def init_audio_server(self):
        ...
        try:
            print("CREATING HARDCODED AUDIO SERVER FOR LOOPBACK AUDIO 2")
            # Hardcoded device settings for Loopback Audio 2
            LOOPBACK_DEVICE_INDEX = 5
            NCHNLS = 4  # Quad setup
            SR = 44100
            BS = 256
            
            # Create the server with our parameters
            try:
                ...
                try:
                    ...
                    # Set device after booting
                    self.server.setOutputDevice(LOOPBACK_DEVICE_INDEX)
                    print(f"Device set to {LOOPBACK_DEVICE_INDEX}")
                    ...
                    try:
                        print("Falling back to default audio setup...")
                        self.server = pyo.Server()
                        self.server.boot()
                        self.server.start()
                    except Exception as e:
                        print(f"Fatal error creating server: {e}")
                        self.server = None
            ...
            QMessageBox.critical(self, "Audio Error", 
                f"Could not initialize audio with Loopback Audio 2: {e}\n\nThe synthesizer will not produce sound.")
            self.server = None
    ...
    def create_global_tab(self):
        ...
        self.audio_status_label = QLabel("Using hardcoded Loopback Audio 2 device\nChannels: 4 (Quad)")
    ...
    def test_audio_channels_ui(self):
        ...
        try:
            ...
            num_channels = 4  # Default to quad
