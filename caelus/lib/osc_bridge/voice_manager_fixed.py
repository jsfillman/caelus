"""
Fixed version of Voice allocation and management for polyphonic synth.
...
This version has enhanced error handling and debugging for voice allocation issues.
"""
import logging
...
class VoiceManagerFixed:
    """
    Fixed version of VoiceManager with improved voice allocation.
    ...
    def __init__(self, voices: List[Voice]) -> None:
        ...
        """
        # IMPORTANT: Make sure voices list is valid
        if not voices:
            LOG.warning("VoiceManager initialized with empty voices list!")
            self.voices = []
        else:
            self.voices = voices
            LOG.info(f"VoiceManager initialized with {len(voices)} voices")
    ...
    def allocate_voice(self, note_num: int) -> Optional[Voice]:
        ...
        """
        # IMPORTANT: Check if we have any voices at all
        if not self.voices:
            LOG.error(f"Cannot allocate voice for note {note_num}: No voices available")
            return None
        
        # Check if note is already playing
        if note_num in self.active_notes:
            voice_idx = self.active_notes[note_num]
            # Make sure voice_idx is valid
            if 0 <= voice_idx < len(self.voices):
                ...
                LOG.error(f"Invalid voice index {voice_idx} for note {note_num}")
                # Continue with new allocation
        
        # Find available voice
        free_voice: Optional[Voice] = None
        for i, voice in enumerate(self.voices):
            LOG.debug(f"Checking voice {i}: active={voice.is_active}, note={voice.note}")
            if not voice.is_active:
                free_voice = voice
                self.note_tracker.assign_note_to_voice(note_num, i)
                LOG.debug(f"Allocated free voice {i} to note {note_num}")
                break
        ...
        free_count = len(self.voices) - len([v for v in self.voices if v.is_active])
        LOG.debug(f"Voice allocation: {active_count} active notes, {free_count} free voices")
    ...
    def note_on(self, note_num: int, velocity: float) -> bool:
        ...
        # IMPORTANT: Check if we have any voices at all
        if not self.voices:
            LOG.error(f"Cannot process note_on for note {note_num}: No voices available")
            return False
    ...
    def note_off(self, note_num: int) -> None:
        ...
        if note_num in self.active_notes:
            voice_idx = self.active_notes[note_num]
            # Check if voice_idx is valid
            if not (0 <= voice_idx < len(self.voices)):
                LOG.error(f"Invalid voice index {voice_idx} for note {note_num}")
                # Remove invalid mapping
                self.note_tracker.release_note(note_num)
                return
    ...
    def _process_note_off(self, note: int, voice_idx: int) -> None:
        ...
        if voice:
            success = voice.note_off()
            if not success:
                LOG.warning(f"Failed to send note-off to voice {voice_idx}")
    ...
    def set_pitch_bend(self, value: float) -> None:
        ...
        for note, voice_idx in self.active_notes.items():
            # Check if voice_idx is valid
            if not (0 <= voice_idx < len(self.voices)):
                LOG.error(f"Invalid voice index {voice_idx} for note {note}")
                continue
